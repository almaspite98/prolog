...
mine1(task(Rows,Cols,Mines,Infos), Bd) :- !,
	length(Bd, Rows), all_length(Bd, Cols), append_lists(Bd, All),
	count(All, Mines), 
	mine_infos(Infos, Bd).
%	labeling(All).
mine1(PId, Bd) :-
	problem(PId, Task), mine1(Task, Bd).

mine_infos([], _).
mine_infos([i(R,C,M)|Infos], Bd) :-
	nth1(R, Bd, Row), nth1(C, Row, 0),
	neighbs(n(R, C, Bd), Ns), count(Ns, M),
	mine_infos(Infos, Bd).
